...
struct AIChatView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 8) {
                VStack(alignment: .leading, spacing: 2) {
                    Text(L10n.get("ai.workspace.title")).font(.headline)
                    Text(cliType.displayName).font(.caption).foregroundStyle(.secondary)
                }
                Spacer(minLength: 0)
                iconButton("ai.workspace.history", "clock") { showingHistory = true }
                    .disabled(isProcessing)
                iconButton("ai.workspace.new", "square.and.pencil") { selectedCardId = nil }
                    .disabled(isProcessing)
                iconButton("ai.workspace.settings", "gearshape") { onOpenSettings() }
            }
            .padding(12)
            Divider()
            transcript
        ...
        .sheet(isPresented: $showingHistory) { historyPage }
    ...
    private var historyPage: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                Spacer()
                Button(L10n.get("common.close")) { showingHistory = false }.keyboardShortcut(.cancelAction)
            }.padding(16)
            ...
            List {
                ForEach(conversationCards.reversed().filter { historyQuery.isEmpty || $0.userMessage.content.localizedStandardContains(historyQuery) }) { card in
                    HStack {
                        Button {
