...
- `KeyboardShortcutManager.swift`와 `AppCommands.swift`: 사용자 단축키 정의·저장 및 메뉴 동작 연결. 사용자 변경이 가능한 액션은 뷰의 고정 키로 우회하지 않는다. 저장 파일은 `~/Library/Application Support/Loreweave/shortcuts.json`이다.
