...
`TextDocument`의 행 모델, `TextSelection`의 위치·범위, `EditorState`의 편집·Undo 상태, `ViewportManager`의 표시 영역이 분리되어 있다. 실제 렌더링과 IME 입력은 `Views/MainEditor/EditorPanel/LoreTextView/`에 있다.
