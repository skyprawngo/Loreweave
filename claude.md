...
각 폴더의 `claude.md`는 AI가 코드베이스를 빠르게 파악하기 위한 참조 문서입니다.

### 작성 규칙
- **간결하게**: 파일 목록, 역할, 핵심 개념만 기술
- **중복 금지**: 코드에서 읽을 수 있는 내용은 생략
- **구조 중심**: 어떤 파일이 어디에 있고, 무슨 기능인지만 명시
- **확장 가이드**: 새 기능 추가 시 어디를 수정해야 하는지만 기술

### 포함할 내용
- 폴더 내 파일/하위 폴더 목록과 역할
- 핵심 타입/클래스 이름과 한 줄 설명
- 새 기능 추가 시 수정할 파일 위치

### 포함하지 않을 내용
- 메서드별 상세 설명 (코드 주석으로 충분)
- 구현 로직 설명 (코드를 읽으면 됨)
- 사용 예시 코드 (필요 시 최소한만)
